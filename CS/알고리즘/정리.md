...
* 별도의 알고리즘은 없고, 구현력이 중요
* Tip
  * 주어진 조건을 되도록 그대로 구현 (나중에 헷갈림)
  * 되도록 쉽게 구현 (최악케이스 - 코드가 복잡해서 디버깅이 안됨, 시간 소모 많아짐)
  * Console.log 찍으면서 확인 연습

# MST (Mininmum Spaning Tree)

* Spanning Tree 는 모든 노드가 연결된 트리
* MST는 최소 비용으로 모든 노드가 연결된 트리
* 풀이 방법 - Kruskal, Prim
  * Kruskal - 전체 간선 중 작은 것 부터 연결
  * Prim - 현재 연결된 트리에 이어진 간선 중 가장 작은 것 추가, 힙자료구조를 사용
    * Heap
      * 최대값, 최소값을 빠르게 계산하기 위한 자료구조 (minHeap, maxHeap)
      * 이진 트리 구조
      * 처음 저장할 때부터 최대값이나 최솟값이 결정되도록 구성
* 시간 복잡도 O(ElogE)
  * Edge 리스트에 저장 O(E)
...
* Tip
  * MST 기본 코드는 우선 암기
  * 해당 문제가 MST문제인지 확인하는 방법, 문제 유형
    * 모드느 노드가 연결되도록 하는 유형
    * 이미 연결된 노드를 최소의 비용으로 줄이는 유형
...
* 준비물
  * 그래프
  * 시작 정점에서 현재 방문한 정점까지의 최단 이동거리를 저장하는 배열
  * 최소힙, 정점과 가중치를 저장
  * 방문 처리여부를 나타낼 배열
* 구현 과정
  1. 초기 세팅
    * 설정된 시작 정점을 방문처리
    * 최단거리 배열의 자기 인덱스(0)에 현재 이동거리 0 삽입
    * 시작점과 인접한 정점들 수만큼 정점 가중치 객체 생성 (인접 정점 데이터, 시작점에서 인접 정점까지 이동한 거리)
    * 위 객체를 방문 처리 여부를 확인 후 방문 처리가 안됐을 시 최소힙 삽입
  2. 최소 힙에서 pop을 한다.(루트 노트에 있는 값을 가져옴)
  3. 루트 노드에서 새로 가져온 정점을 검사한다.
    * 방문 여부 검사, 안했다면 방문 처리
    * 최단 거리 배열에서 자신의 인덱스(새로 가져온 값)요소에 가중치 속성값을 삽입 (즉, 두번째 요소)
    * 인접한 정점의 수만큼 정점 가중치 객체를 동일하게 생성 (인접 정점 데이터, 자신의 위치까지 최단거리 + 가중치)
    * 방문 여부 확인 후 최소 힙 객체 삽입
  4. 최소 힙의 데이터가 하나도 없을 때 까지 1 - 3 과정 반복
...
class minHeap {
  ...
  const [V, E] = [정점의 개수, 간선의 개수]
  const K = 출발노드
  const graph = Array.from({length: V + 1}, () => []); // 정점의 개수만큼 그래프 안에 배열 만들기
  const isVisited = new Array(V + 1).fill(false); // 방문 여부 판단 배열
  const heap = new minHeap(); // 힙
  const dist = Array.from({length: V + 1}, () => Infinity); // 최단 이동거리 저장 배열

  for  (let i = 2; i < E + 2; i++) {
    const [start, end, weight] = [간선 정보].split(" ").map(v => +v);
    graph[start].push([end, weight]);
  }

  heap.add([K, 0]); // 출발 노드 입력
  dist[K] = 0;

  while(heap.size()) {
    const [vertex, weight] = heap.remove();

    if (dist[vertex] !== weight) continue; // 이미 dist 배열내에서 변경됐다면 (다른 더 작은 거리로)

    graph[vertex].forEach((v) => {
      const [dst, weight] = v;
      if (!isVisited[dst] && dist[dst] > dist[vertex] + weight) { // 최단거리 더 작으면 교체
...
* 다익스트라로 모든 노드를 탐색하면 => O(V^3logV) 
* 작동 방식
  1. 이차원 배열로 비용 구성 (초기값은 INF)
  2. 간선의 값을 비용 배열에 반영
  3. 모든 노드에 대해 해당 노드를 거쳐서 갈 때 비용이 작아진다면 값 갱신
* TIP
  * 그래프 거리 문제 경우
    * 한점 -> 여러점 : 다익스트라 (ElogV)
    * 여러 점 -> 여러점 : 플로이드 (V^3)
    * 노드가 적다면 플로이드, 간선이 적다면 다익스트라 (시간복잡도 따로 계산 필수지만 대체로)
### JS 구현
```
const N = 노드의 갯수;
const M = 간선의 갯수;
...
for (let i = 1; i < n + 1; i++) {
  result[i][i] = 0; //자기 자신으로 가는 길 0으로
}

for (let i = 1; i <= M; i++) {
  const [start, destination, cost] = 받아온값;
